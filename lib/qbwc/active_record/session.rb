class QBWC::ActiveRecord::Session < QBWC::Session
  ...
  def initialize(session_or_user = nil, company = nil, ticket = nil, account_id = nil)
    if session_or_user.is_a? QbwcSession
      ...
      @pending_jobs = ar_refresh_pending_jobs(@session.pending_jobs.split(','), account_id, self).join(',')
  ...
  def ar_refresh_pending_jobs(pending_job_names, account_id, session)
    QBWC.get_pending_jobs_by_name(@session.pending_jobs, account_id, self)
  end

  def save
    @session.pending_jobs = ar_refresh_pending_jobs(pending_jobs.map(&:name), nil, self).join(',')
